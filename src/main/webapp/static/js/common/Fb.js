var Fb = {
    ...
    baseAjax: function (url, tip) {
        var ajax = new $ax(Fb.ctxPath + url, function (data) {
            Fb.success(tip + "成功!");
        }, function (data) {
            Fb.error(tip + "失败!" + data.responseJSON.message + "!");
        });
        return ajax;
    },
    changeAjax: function (url) {
        return Fb.baseAjax(url, "修改");
    },
        ...
        $.ajaxSetup({
            ...
            complete: function (XMLHttpRequest, textStatus) {
                ...
                if (sessionstatus == "timeout") {
                    //如果超时就处理 ，指定要跳转的页面
                    window.location = Fb.ctxPath + "/global/sessionError";
                }
            }
        });
    },
    ...
    newCrontab: function (href, menuName) {
        ...
        if (needCreateCrontab) {
            ...
            var iframeContent = '<iframe class="J_iframe" name="iframe' + Fb.randomNum(100,999) + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
